...
class Solver(BaseSolver):
    ...
    def run(self, callback):
        ...
        while callback(u):
            ...
            if self.data_fit == 'huber':
                w = w_tmp - sigma_w * (self._prox_huber(w_tmp/sigma_w - self.y, 1./sigma_w) + self.y)
            else:
                w = (w_tmp - sigma_w *  self.y) / (1. + sigma_w)
    ...
    def _prox_huber(self, u, mu):
        return np.where(np.abs(u) <= self.delta * (mu + 1.),
                        u / (mu + 1.), u - self.delta * mu * np.sign(u))
